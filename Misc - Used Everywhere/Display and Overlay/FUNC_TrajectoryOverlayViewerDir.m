function [] = FUNC_TrajectoryOverlayViewerDir(TRAJECTORY, DATA_PATH, IMAGE_TYPE, SAVE_DATA, FRAMES_TOTAL)
%This function plots given trajectory locations (in TRAJECTORY) over the
%images trajectories are sampled from, located in DATA_PATH. The number of
%time points is extracted from the number of files of the specified
%IMAGE_TYPE, so all data files should be isolated in a designated folder.
%IMAGE_TYPE should be a string spelling the extension of your images.
...
%Extract all files of type IMGTYPE located in DATA_PATH
if ~contains(IMAGE_TYPE, '.')
    IMAGE_TYPE = ['.' IMAGE_TYPE];
end
imgStruct = dir(fullfile(DATA_PATH, ['*' IMAGE_TYPE]));

%Define total number of files to be analyzed
if nargin == 4
    FRAMES_TOTAL = size(imgStruct,1);
elseif nargin == 5
    FRAMES_TOTAL = min(FRAMES_TOTAL, size(imgStruct,1));
end
...
for currFrame = 1:FRAMES_TOTAL
    ...
    hold off
    currFrameDir = fullfile(DATA_PATH, imgStruct(currFrame).name);
    currImg = imread(currFrameDir);
    imshow(currImg);
    hold on
