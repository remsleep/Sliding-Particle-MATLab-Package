function [REL_PARVEL, REL_PERPVEL, DISTANCE, COORD_MATRIX, DELTAANG, FRAMES, CHANNELS] = ...
    JUDE_PairwiseParPerpVelocitiesSameChannelArray(velInfo, WINDOW, cutOffAngle)
...
for currFrame = 1:max(FRAME)
    ...
    if sum(trajsInFrame) > 0
        ...
        %Track whether the MTs in each pair are from different or same
        %channels
        tempChannel = CHANNEL(trajsInFrame);
        currChannels = triu((tempChannel(:).'+ tempChannel(:))');
...
%         relPerpVels = relPerpVels.*sign(diffPerpCoordMat);
        ...
        CHANNELS(countInd:(countInd+sizeData-1)) = currChannels(indexMat);
...
%         diffParCoordMat = triu(tempParCoord1 - tempParCoord2);
%         diffPerpCoordMat = triu(tempPerpCoord1 - tempPerpCoord2);
%         
